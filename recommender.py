...
class MovieRecommender:
    ...
    def train_model(self, ratings_df, epochs=50, batch_size=1024, learning_rate=0.001, patience=5):
        ...
        val_dataset = MovieRatingDataset(val_df, self.user_to_idx, self.movie_to_idx)
        ...
        for epoch in range(epochs):
            ...
            with torch.no_grad():
                for user_idx, movie_idx, rating in val_loader:
                    ...
                    output = self.model(user_idx, movie_idx)
                    val_loss = criterion(output, rating.squeeze())
        ...
        if best_model_state is not None:
            ...
            print(f"Restored best model from epoch with validation loss: {best_val_loss:.4f}")
        ...
        with torch.no_grad():
            for user_idx, movie_idx, rating in test_loader:
                ...
                output = self.model(user_idx, movie_idx)
                test_loss += criterion(output, rating.squeeze()).item()
        ...
        # Store learning curves for analysis
    ...
    def plot_learning_curves(self, show_plot=True, save_plot=False, filename='learning_curves.png'):
        ...
        if not hasattr(self, 'train_losses') or not hasattr(self, 'val_losses'):
            print("No learning curves available. Train the model first.")
            return
        ...
        try:
            ...
            # Find optimal epoch
            optimal_epoch = np.argmin(self.val_losses) + 1
            ...
            if len(self.val_losses) > 5:
                recent_val_trend = np.mean(self.val_losses[-3:]) - np.mean(self.val_losses[-6:-3])
                if recent_val_trend > 0.01:
                    print(f"  ⚠️  Potential overfitting detected (validation loss increasing)")
                else:
                    print(f"  ✅ No significant overfitting detected")
                    
        except ImportError:
            print("Matplotlib not available. Install with: pip install matplotlib")
            print("Learning curves data is still available in self.train_losses and self.val_losses")
            # Still provide text analysis even without matplotlib
            self._print_learning_analysis()
    
    def _print_learning_analysis(self):
        """Print learning curve analysis without plots."""
        if not hasattr(self, 'train_losses') or not hasattr(self, 'val_losses'):
            return
            
        optimal_epoch = np.argmin(self.val_losses) + 1
        print(f"\nLearning Curve Analysis (Text Only):")
        print(f"  Optimal epoch: {optimal_epoch}")
        print(f"  Best validation loss: {min(self.val_losses):.4f}")
        print(f"  Final training loss: {self.train_losses[-1]:.4f}")
        print(f"  Final validation loss: {self.val_losses[-1]:.4f}")
    ...
    def display_recommendations(self, recommendations):
        ...
        print("-"*100)
        print(f"Generated {len(recommendations)} recommendations based on your ratings.")
        print("="*100)
...
def main():
    ...
    # Plot learning curves with options
    # Default: show plot, don't save
    recommender.plot_learning_curves()
    
    # Alternative options:
    # recommender.plot_learning_curves(show_plot=False, save_plot=True)  # Save only
    # recommender.plot_learning_curves(show_plot=True, save_plot=True, filename='my_curves.png')  # Both
    # recommender.plot_learning_curves(show_plot=False, save_plot=False)  # Text analysis only
